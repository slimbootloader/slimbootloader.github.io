...
.. note:: Intel\ |reg| Core\ |trade| Ultra Series 3 Processor, formally known as |PTL| family.

Supported Boards
^^^^^^^^^^^^^^^^^^^^^

|SPN| supports various platforms corresponding to |PTL|.

Each |PTL| board is assigned with a unique platform ID.

  +-------------------------+---------------+-------------------+---------------+------------+
  |        Board            |  Platform ID  | SPI Programmer    |     UART      |     PLAT   |
  +-------------------------+---------------+-------------------+---------------+------------+
  |      |PTL| DDR5 RVP     |     0x0004    |      J1H1         |     J8J3      |     ptl    |
  +-------------------------+---------------+-------------------+---------------+------------+
  |      |PTL| DDR5 CRB     |     0x0005    |      J28          |     J60       |     ptl    |
  +-------------------------+---------------+-------------------+---------------+------------+
  |      |PTL| LPDDR5 T3    |     0x0001    |      J1H1         |     J8J3      |     ptl    |
  +-------------------------+---------------+-------------------+---------------+------------+
  |      |PTL| LPDDR5 T4    |     0x0003    |      J1H1         |     J8J3      |     ptl    |
  +-------------------------+---------------+-------------------+---------------+------------+
...
For |PTL| platforms, serial port connector location can be found from the above table for each supported target board.
...
To build |SPN| for any |PTL| platform::
...
1. Gather |PTL| IFWI firmware image
  ...
  This image contains additional firmware ingredients that are required boot on |PTL|.
...
2. Stitch |SPN| images into downloaded BIOS image::
    ...
    python Platform/PantherlakeBoardPkg/Script/StitchLoader.py -i <BIOS_IMAGE_NAME> -s Outputs/<plat>/SlimBootloader.bin -o <SBL_IFWI_IMAGE_NAME>

  where -i = Input file, -o = Output file, plat = ptl

For example, to stitch |SPN| IFWI image ``sbl_ptl_ifwi.bin`` from |PTL| downloaded firmware images::

    python Platform/PantherlakeBoardPkg/Script/StitchLoader.py -i xxxx.bin -s Outputs/ptl/SlimBootloader.bin -o sbl_ptl_ifwi.bin
...
Flash the generated ``sbl_ptl_ifwi.bin`` to the target board using a DediProg SF100 or SF600 programmer.
...
For all |PTL| platforms, the below command can be used::
...
|SPN| for |PTL| uses BIT16 of PMC I/O register (Over-Clocking WDT Control (OC_WDT_CTL) - Offset 54h) to trigger firmware update. When BIT16 is set, |SPN| will set the boot mode to FLASH_UPDATE.
